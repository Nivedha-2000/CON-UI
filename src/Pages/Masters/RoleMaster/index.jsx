...
function RoleMaster({ name }) {
    ...
    const getDataById = roleId => {
        return ApiCall({
            path: API_URLS.GET_ROLE_MASTER_BY_ID + "/" + roleId,
        })
    }
    const add = async () => {
        try {
    ...
    const edit = async (roleId, type) => {
        try {
            setLoader(true)
            setVisible(true);
            let { data } = (roleId && await getDataById(roleId))
            if (!data) {
    ...
    return (
        <div className='defect-master-main'>
            ...
            } title={< h6 className='m-0' > {`${fields.id === 0 ? "Add New" : "Edit"} Role`}</h6 >} placement="right" onClose={() => {
                clearFields();
                onClose();
            }} visible={visible} >
