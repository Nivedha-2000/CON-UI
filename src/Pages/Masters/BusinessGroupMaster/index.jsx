...
const initialErrorMessages = {
    ...
    requiredFields = ["bGcode", "bgName"]

function BusinessGroupMaster({ name }) {
    const clearFields = () => {
        setFields({
            ...initialFieldValues
        });
    ...
    const [bGcode, setlbGcodeList] = useState([]);
    const [bgName, setbgNameList] = useState([]);
    ...
    const getDatas = () => {
        setListLoading(true)
        ApiCall({
            path: API_URLS.GET_BUSINESS_GROUP_MASTER_LIST
        }).then(resp => {
    ...
    return (
        <div className='defect-master-main'>
            ...
            } title={< h6 className='m-0' > {`${fields.id == 0 ? "Add New" : "Edit"} Business Group Master`}</h6 >} placement="right" onClose={onClose} visible={visible} maskClosable={false} >
...
BusinessGroupMaster.propTypes = {
    name: PropTypes.string
}


export default BusinessGroupMaster;